-- Ported mythic-targeting here as a compat layer
-- exact same Mythic events the original menus did
-- Disable with:  setr ox_target:mythicMenus 0

if GetConvarInt('ox_target:mythicMenus', 1) ~= 1 then return end

local compat = require 'client.compat.mythic-targeting'
local api = require 'client.api'
local translate = compat.translate

local PlayerPedId = PlayerPedId

-- Mythic components used by the option bodies below (fetched lazily; the option
-- closures capture these upvalues, so they update once fetched).
local Vehicles, Mechanic, Tow, Polyzone
...
local function fetchComponents()
    ...
    Vehicles = get('Vehicles') or Vehicles
    Mechanic = get('Mechanic') or Mechanic
    Tow = get('Tow') or Tow
    Polyzone = get('Polyzone') or Polyzone
end

-- IsNearTrunk helper (ported from mythic-targeting/client/utils.lua).
local function trunkOffset(ent)
    local min = GetModelDimensions(GetEntityModel(ent))
    return GetOffsetFromEntityInWorldCoords(ent, 0.0, min.y - 0.5, 0.0)
end

local function IsNearTrunk(ent, dist, open)
    return #(trunkOffset(ent) - GetEntityCoords(PlayerPedId())) <= (dist or 1.0)
...
local function registerMenus()
    api.addGlobalVehicle(translate(Config.VehicleMenu, 'fa-solid fa-car', 'vehicle', nil, 7))
    api.addGlobalPlayer(translate(Config.PlayerMenu, 'fa-solid fa-user', 'player', nil, 7))
end

AddEventHandler('Core:Shared:Ready', fetchComponents)

-- Keep trying until the Vehicles component is available (it is registered by
-- mythic-vehicles, which may load after this resource).
CreateThread(function()
    for _ = 1, 100 do
        fetchComponents()
        if Vehicles then break end
        Wait(500)
    end
end)
